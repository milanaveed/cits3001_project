...
class CustomRewardAndDoneEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Checks if mario stays at the same x position, still under experiment
...
DATA_DIR = './reinforcement-learning/train1-1-v3/'
    ...
    def _on_step(self):
        ...
        if self.n_calls % self.check_freq == 0:
            ...
            model_path = os.path.join(
                self.save_path, '1-1_v3model_{}'.format(self.n_calls))  # Saves the model
            ...
            # Record the current time step of each episode
            total_time = [0] * NUMBER_OF_TRIALS
            # Record x position when each episode ends
            ...
            with open(DISTANCE_LOG_PATH, 'a') as f:
                print(self.n_calls, ',', np.mean(x_position),
                      ',', best_x_position, file=f)

            with open(TRAINING_TIME_LOG_PATH, 'a') as f:
                print(self.n_calls, ',', elapsed_time, file=f)

            with open(PASS_RATE_LOG_PATH, 'a') as f:
                print(self.n_calls, ',', round(
                    number_of_pass/NUMBER_OF_TRIALS*100), file=f)

        return True


callback = TrainAndSaveCallback(check_freq=10000, save_path=DATA_DIR)
model = PPO('CnnPolicy', env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log=DATA_DIR,
            learning_rate=0.0001, n_steps=512, batch_size=64, n_epochs=10, gamma=0.9, gae_lambda=1.0, ent_coef=0.01)
model.learn(total_timesteps=1000000, callback=callback)

model.save('1-1-v3model')
